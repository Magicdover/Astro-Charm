...
- 🌈 **五套可切换配色**（默认 / 羊皮纸 / 松林 / 海蓝 / 樱粉），与明暗自由组合，均保证文字清晰可读
- 🖼️ **可切换背景图** + **背景特效**（代码雨 / 蛛网 / 飞雪），右下角设置面板一键调整
- 🧭 丰富的侧边栏导航：关于、项目、世界杯、网址导航、壁纸、一言、友链
- 🖱️ 内容页右上角**关闭按钮**，带动画一键返回导航，告别浏览器后退
...
- **Windows**：双击根目录的 **`启动.bat`** 即可启动本地预览。脚本会自动检测 Node.js、选择包管理器（优先 pnpm）、首次运行时自动安装依赖，随后打开浏览器访问 `http://localhost:4321`。
...
astro-website/
├── 启动.bat                        # 一键脚本
├── astro.config.mjs                # 站点与主题配置 + 增强脚本
├── src/
│   ├── content/
│   │   ├── posts/                  # 博客文章
│   │   └── specials/               # 侧边栏页面（关于 / 项目 / 网址导航 / 壁纸 / 一言…）
│   ├── enhance/enhance.js          # 设置面板 + 关闭按钮 + 背景特效
│   ├── styles/custom-charm.css     # 自定义样式
│   └── assets/                     # 图片等资源
└── public/
    ├── backgrounds/                # 背景图（设置面板切换）
